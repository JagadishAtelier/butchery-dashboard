...
import { ArrowLeft, X } from "lucide-react"; // removed MapPin import
        ...
        <div className="fixed right-6 top-28 z-50 w-96 bg-white border rounded-xl shadow-2xl overflow-hidden">
          ...
          <div className="p-3 border-t">
            <div className="text-xs text-gray-600 mb-2">
              ...
              <div>{order.pingedAt ? formatDateStr(order.pingedAt) : "No timestamp"}</div>
              <div className="mt-1">
                Lat: {pingLatLng.lat.toFixed(6)}, Lng: {pingLatLng.lng.toFixed(6)}
              </div>
            </div>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white shadow rounded-xl p-6 border-l-4 border-indigo-500 hover:shadow-lg transition">
              ...
              <p><span className="font-medium">Name:</span> {order.buyer?.name || "N/A"}</p>
              <p><span className="font-medium">Email:</span> {order.buyer?.email || "N/A"}</p>
              <p><span className="font-medium">Location:</span> {order.location || "N/A"}</p>
              <p><span className="font-medium">Instructions:</span> {order.deliveryInstructions || "N/A"}</p>
            </div>
            <div className="bg-white shadow rounded-xl p-6 border-l-4 border-green-500 hover:shadow-lg transition">
              ...
              <p><span className="font-medium">Paid On:</span> {order.paymentDate ? new Date(order.paymentDate).toLocaleString() : "N/A"}</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {order.products.map((p) => {
              ...
              const imageUrl = (productInfo?.images && productInfo.images[0]) || productInfo?.image || "/placeholder.png";
              return (
                <div key={p._id} className="flex gap-4 items-center bg-white shadow rounded-xl p-4 hover:shadow-xl transition border border-gray-100">
                  <div className="w-24 h-24 flex-shrink-0 rounded-lg overflow-hidden">
                    <img src={imageUrl} alt={p.productId?.name || productInfo?.name} className="w-full h-full object-cover" onError={(e)=> (e.target.src="/placeholder.png")} />
                  </div>
                  <div className="flex-1 flex flex-col justify-between h-full">
                    <div>
                      <h3 className="font-semibold text-gray-800">{p.productId?.name || productInfo?.name || "Product"}</h3>
                    ...
                    <div className="text-right">
                      ...
                      <p className="font-bold text-indigo-600 text-lg">Subtotal: ₹{(p.price || 0) * (p.quantity || 1)}</p>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="">
            <div className="bg-white shadow-lg rounded-xl p-6 border-t-4 border-indigo-500">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between font-bold text-indigo-600 text-lg mt-2"><span>Final Amount</span><span>₹{order.finalAmount}</span></div>
              </div>
              <div className="mt-6">
                <span className={`px-3 py-1 rounded-full font-medium text-sm ${order.status === "Pending" ? "bg-yellow-100 text-yellow-800" : "bg-green-100 text-green-800"}`}>{order.status}</span>
              </div>
            </div>
          </div>

          {/* Main map */}
          <div ref={mapSectionRef} id="order-map" className="bg-white shadow rounded-xl p-4 border border-gray-100">
            ...
            {isClient && pingLatLng ? (
              <>
                    ...
                    <Marker position={[pingLatLng.lat, pingLatLng.lng]}>
                      <Popup>
                        <div>
                          <div className="font-medium">Ping</div>
                          <div>{order.pingedAt ? formatDateStr(order.pingedAt) : "No timestamp"}</div>
                          <div>Lat: {pingLatLng.lat.toFixed(6)}, Lng: {pingLatLng.lng.toFixed(6)}</div>
                        </div>
                      </Popup>
                    </Marker>
                  </MapContainer>
                ...
                <div className="mt-3 flex gap-2">
                  <a href={googleMapsLink} target="_blank" rel="noreferrer" className="px-3 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 text-sm">Open in Google Maps</a>
                  <button onClick={() => safeCopy(`${pingLatLng.lat},${pingLatLng.lng}`)} className="px-3 py-2 bg-gray-100 rounded hover:bg-gray-200 text-sm">Copy Coordinates</button>
                </div>
              </>
            ) : (
